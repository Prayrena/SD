...
ObjModel::ObjModel(Vec3 const& pos /*= Vec3::ZERO*/)
	: BaseModel(pos)
{
...
void ObjModel::Render() const
{
	...
	if (m_renderDebug && !m_debugVertexes.empty())
	{
		g_theRenderer->SetBlendMode(BlendMode::OPAQUE);
		g_theRenderer->SetRasterizerMode(RasterizerMode::SOLID_CULL_BACK);
		g_theRenderer->SetDepthMode(DepthMode::ENABLED);
...
bool ObjModel::LoadXml(std::string const& fileName)
{
	XmlDocument objXml;
...
bool ObjModel::LoadObj(std::string const& fileName, Mat44 const& transform /*= Mat44()*/)
{
...
Material* ObjModel::GetMaterial() const
{
	return m_material;
}

void ObjModel::CreateDebugTangentBasisVectors()
{
...
void ObjModel::CreateDebugVertexBuffer()
{
