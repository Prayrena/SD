...
Model::Model(Game* game, std::string fileName /*= "Undefined"*/)
	: Entity(game)
	, m_objectFileName(fileName)
{
	Strings fileNameStrings = SplitStringOnDelimiter(m_objectFileName, '.');
	std::string fileType = fileNameStrings.back();
	if (fileType == "xml")
	{
		LoadXml(m_objectFileName);
	}
	else if (fileType == "obj")
	{
		LoadObj(m_objectFileName);
	}
}
...
bool Model::LoadXml(std::string const& fileName)
{
	...
	{
		//Strings filePathTokens = SplitStringOnDelimiter(materialXMLFilePath, '/');
		//std::string xmlFname;
		//for (int charIndex = 0; charIndex < filePathTokens[2].size(); ++charIndex)
		//{
		//	char singleChar = filePathTokens[2][charIndex];
		//	while (singleChar != '.')
		//	{
		//		xmlFname.push_back(singleChar);
		//	}
		//	continue;
		//}
		//
		//const char* xmlExt = ".xml";
		//const char* xmlFnameChar = xmlFname.c_str();
		//
		//_makepath_s(
		//	matXMLFilePath,
		//	sizeof(matXMLFilePath),
		//	drive,
		//	dir,
		//	xmlFnameChar,
		//	xmlExt);
