...
bool IsMostlyEqual(Quat const& a, Quat const& b, bool acceptOpposite);
bool IsMostlyEqual(Quat const& a, float x, float y, float z , float w, bool acceptOpposite);

bool IsMostlyEqual(Quat const& a, Quat const& b, bool acceptOpposite)
{
	return IsMostlyEqual(a, b.x, b.y, b.z, b.w, acceptOpposite);
}

bool IsMostlyEqual(Quat const& a, float x, float y, float z, float w, bool acceptOpposite)
{
	if (IsMostlyEqual(a.x, x) && IsMostlyEqual(a.y, y) && IsMostlyEqual(a.z, z) && IsMostlyEqual(a.w, w))
	{
		return true;
	}

	if (acceptOpposite && IsMostlyEqual(a.x, -x) && IsMostlyEqual(a.y, -y) && IsMostlyEqual(a.z, -z) && IsMostlyEqual(a.w, -w))
	{
		return true;
	}
...
int TestSet_Custom_Quat()
{
	Quat quatInPhysX; // = TransformQuatFromGameToPhysX(Quat());
	VerifyTestResult(IsMostlyEqual(quatInPhysX, 0.f, 0.f, 0.f, 3.f, false), "Identity Quat is incorrect"); // e.g. a non-graded test that doesn't pass
...
void RunTests_Custom()
{
	...
	RunTestSet( false, TestSet_Custom_Quat,		"Custom quat sample tests" );
}
