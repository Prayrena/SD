//-----------------------------------------------------------------------------------------------
// ProtogameCmd: Main.cpp (version 4.1) - for SMU Guildhall assignments MP1-A1 through MP1-A7
//
...
OpenXrProgram* g_theApp = nullptr;
...
int main(int argc, char* argv[]) {
    try {
        ...
        do {
            ...
            // Initialize the OpenXR program.
            g_theApp = new OpenXrProgram(options, platformPlugin);

			g_theApp->CreateInstance();
			g_theApp->InitializeSystem();

            if (g_theApp->m_openXRAvaible)
            {
				options->SetEnvironmentBlendMode(g_theApp->GetPreferredBlendMode());
            ...
            while (!quitKeyPressed && !g_theApp->m_isQuitting) 
            {
				if (g_theApp->m_openXRAvaible)
				{
					bool exitRenderLoop = false;
					g_theApp->PollEvents(&exitRenderLoop, &requestRestart);
					if (exitRenderLoop)
					{
						break;
					}
